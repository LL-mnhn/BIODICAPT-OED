...
# Introduction
## OED
**Optimal Experimental Design** (or OED for short[^1]) is a method for designing experiments that aims at maximising information gain while minimising a cost function. This cost function can encompass uncertainty, cost, time... it is up 
...
This concept can be particularly interesting when working in participartory science, which is our case for the BIODICAPT project.

[^1]: Sometimes, this concept is also referred to as "adaptive sampling".
...
![Flowchart of the proof of concept conducted with STOC dataset](images/Flowchart-STOC-data.png){height=1000}
...
Mondain-Monval et al. [@mondainmonval_adaptive_2024] show results in boxplots, for several proportion of their modified. We show results for the same addition of training points, but divided into train/val/test subsets. 

In our results, differences in MSE between resampling strategies are slight, less marked than those of [@mondainmonval_adaptive_2024]. However, we note that the "BAU" strategy yields to a more visible decrease in MSE (aka decrease of errors). 

It is also a surprise to note that the strategy "SU" does not yield to the best results (which was what we expected from [@mondainmonval_adaptive_2024]). When looking at the boxplots for the training scores of "SU", an explanation could be that the model was confronted to much variability (compared other training subsets), which as a sligthly negative impact on predictions in test. This is actually quite logical: the "SU" strategy adds training samples for which the predictions of the model are the most uncertain, which is why variability is wider.


#### What to conclude?

Subset comparisons confirm that training is harder with new unknown samples, but validation scores are better, as well as test scores. However, the differences with the base strategy is not statitically significant.

Overall, the addition of new points in the sampling strategies has slight a positive effect on the predictions of the models on the test set. The best performing strategy on our practical dataset, using HMSC, seems to be "gap-filling" (GF). 
