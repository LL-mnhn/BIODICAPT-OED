...
check_BIODICAPT_files <- function(verbose = VERBOSE) {
    ...
    if (!file.exists(BIODICAPT_FOLDER)) {
        cli_alert_danger(paste0("Folder '", BIODICAPT_FOLDER, "' was not found!\n"))
        stop("Tip: did you create the necessary folders for the datasets files?")
    }
    ...
    for (filename in BIODICAPT_FILENAMES) {
        if (!file.exists(file.path(BIODICAPT_FOLDER, filename))) {
            cli_alert_danger(paste0("File '", filename, "' was not found in '", 
                BIODICAPT_FOLDER,"'!\n"))
            cli_alert_warning("This dataset can only be obtained from the coordinators of the BIODICAPT project. Contact them directly.")
...
check_ENI500_files <- function(verbose = VERBOSE) {
    ...
    if (!file.exists(file.path(ENI500_FOLDER, ENI500_FILENAME))) {
        cli_alert_danger(paste0("File '", filename, "' was not found in '", 
            ENI500_FOLDER,"'!\n"))
        cli_alert_warning("This dataset can only be obtained from the coordinators of the 500 ENI network. Contact them directly.")
...
check_CORINE_raster <- function(verbose = VERBOSE) {
    ...
    if (!file.exists(CORINE_FILEPATH)) {
        cli_alert_danger(paste0("File '", CORINE_FILEPATH, "' was not found!\n"))
...
check_CHELSA_rasters <- function(dataset, verbose = VERBOSE) {
    ...
    for (CHELSA_tiff_path in CHELSA_12_paths){
        if (!file.exists(CHELSA_tiff_path)){
            cli_alert_danger(paste0("File '", CHELSA_tiff_path, "' was not found!\n"))
            cli_alert_warning("If needed, download files from https://www.chelsa-climate.org/datasets/chelsa_monthly.")
            stop("Tip: download monthly CHELSA files for your dataset and put them together in the same subfolder.")
        }
    }
...
check_species_data <- function(verbose = VERBOSE) {
    if (verbose) {
