...
if (authorize_overwrite(PATH_LOCAL_STRAT)) {
    unlink(PATH_LOCAL_STRAT, recursive=TRUE)
    dir.create(PATH_LOCAL_STRAT)
} else {
...
. <- compute_hmsc_performances(
        parent_folder = PATH_LOCAL_STRAT, 
        filename = "strategies_performances.pdf",
        prefix = "model_", 
        model_types = STRATEGIES, 
        sufix = paste0("_random-", R_EFFECT,"_k"), 
        k_fold = K_FOLD, 
        xlabel = "Effect of sampling strategy on metrics", 
